...
class NuclearFusionChecker:
    ...
    def all_increasing_decreasing_by_one_or_two_three(self, fusion_data):
        ...
        for report in fusion_data:
            if self.is_always_increasing_by_one_or_two_three(report) or self.is_always_decreasing_by_one_or_two_three(
                report
            ):
    ...
    def all_increasing_decreasing_by_one_or_two_or_three_with_skip(self, fusion_data):
        ...
        for report in fusion_data:
            if self.is_always_increasing_by_one_or_two_three_with_skip(
                report
            ) or self.is_always_decreasing_by_one_or_two_three_with_skip(report):
                num_of_safe_reports += 1
...
if "__main__" == __name__:
    ...
    # task 2
    num_of_safe_reports_with_skip = fusion_checker.all_increasing_decreasing_by_one_or_two_or_three_with_skip(
        fusion_data
    )
